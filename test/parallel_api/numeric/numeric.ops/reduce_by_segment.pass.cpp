...
{
    // create buffers
    sycl::buffer<ValueType, 1> key_buf{ sycl::range<1>(13) };
    sycl::buffer<ValueType, 1> val_buf{ sycl::range<1>(13) };
    sycl::buffer<ValueType, 1> key_res_buf{ sycl::range<1>(13) };
    sycl::buffer<ValueType, 1> val_res_buf{ sycl::range<1>(13) };
...
test_with_usm()
{
    ...
    auto prepare_data = [](int n, ValueType* key_head, ValueType* val_head, ValueType* key_res_head, ValueType* val_res_head)
        {
            for (int i = 0; i < (n - 1); ++i)
            {
                key_head[i] = i % 4 + 1;
    ...
    constexpr int n = 13;
    ValueType key_head_on_host[n] = {};
    ValueType val_head_on_host[n] = {};
    ValueType key_res_head_on_host[n] = {};
    ValueType val_res_head_on_host[n] = {};

    prepare_data(n, key_head_on_host, val_head_on_host, key_res_head_on_host, val_res_head_on_host);

    // allocate USM memory and copying data to USM shared/device memory
    TestUtils::usm_data_transfer<alloc_type, ValueType> dt_helper1(q, std::begin(key_head_on_host),     std::end(key_head_on_host));
    TestUtils::usm_data_transfer<alloc_type, ValueType> dt_helper2(q, std::begin(val_head_on_host),     std::end(val_head_on_host));
    TestUtils::usm_data_transfer<alloc_type, ValueType> dt_helper3(q, std::begin(key_res_head_on_host), std::end(key_res_head_on_host));
    TestUtils::usm_data_transfer<alloc_type, ValueType> dt_helper4(q, std::begin(val_res_head_on_host), std::end(val_res_head_on_host));
