...
public:
    ...
    TestStdRealImag(TErrorEngine& ee)
        : errorEngine(ee)
    {
    }
    ...
    void run_test()
    {
        ...
        {
            const auto complex_val = dpl::complex<float>(COMPLEX_REAL_PART, COMPLEX_IMAG_PART);
            auto val = dpl::real(complex_val);
                ...
                [&](cl::sycl::handler& cgh)
                {
                    ...
                    cgh.single_task<class TestKernel1>(
                        [=]()
                        {
                            ...
                            // Run test in kernel
                            TestStdRealImag<ErrorEngine_KernelPart_Impl, ::std::true_type, ::std::false_type> tcc(error_engine_kernel_part);
                            tcc.run_test();
                        });
                });
        }
                ...
                [&](cl::sycl::handler& cgh)
                {
                    ...
                    cgh.single_task<class TestKernel2>(
                        [=]()
                        {
                            ...
                            // Run test in kernel
                            TestStdRealImag<ErrorEngine_KernelPart_Impl, ::std::false_type, ::std::false_type> tcc(error_engine_kernel_part);
                            tcc.run_test();
                        });
                });
        }
...
main()
{
    ...
    // Run test on host
    TestStdRealImag<TestUtils::ErrorEngineHost, ::std::true_type, ::std::true_type> tcc(error_engine_host);
    tcc.run_test();
